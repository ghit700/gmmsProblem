function tranTime(timeStr) {
    ...
    var time = new Date();
    if ((api.systemType == 'android' && timeStr.indexOf('-') >= 0) || (api.systemType != 'android' && typeof timeStr == 'string')) {
        time = new Date(Date.parse(timeStr.replace(/-/g, "/")));
    } else {
    ...
    if (timeAgo < 60) {
        ...
        str = showDay(time);
    } else if (timeAgo < 60 * 60 * 24 * 365 && time.getFullYear() == curTime.getFullYear()) {
